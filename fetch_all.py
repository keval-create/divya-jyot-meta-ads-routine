...
# ---------- META ----------
_raw_id = os.environ.get("META_AD_ACCOUNT_ID", "act_XXXXXXXXXX")
AD_ACCOUNT_ID = _raw_id if _raw_id.startswith("act_") else f"act_{_raw_id}"
...
def google_clients():
    ...
    # GOOGLE_SA_JSON may itself be base64-encoded; try raw JSON first, then b64
    try:
    ...
    sheets = build("sheets", "v4", http=authed_http)
    return sheets
